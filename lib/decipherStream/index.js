...
const decipherStream = (inputStream, outputStream, secretKey) => new Promise((resolve, reject) =>
{
    try
    {
        ...
        pipeline(
            ...
            (err) => {
                if (err)
                {
                    reject(new Error('Error en lek-cryptools cuando se intentaba descifrar el stream: ' + err.message));
                }
                else
                {
                    resolve()
    ...
    {
        reject(new Error('Error en lek-cryptools cuando se intentaba descifrar la clave: ' + err.message));
    }
});
